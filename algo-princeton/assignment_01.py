...
all members are connected (i.e., every member is a friend of a friend of a friend ...
  of a friend).

Assume that the log file is sorted by timestamp and that friendship is an equivalence
relation. The running time of your algorithm should be m log n
or better and use extra space proportional to n.

Answer: using a weighted graph to implement this
'''
class SN:
  network = []

  def friend(self, a, b):
    self.network[self.root(a)] == self.root(b)

  def __init__(self, size):
    self.network = [i for i in range(size)]
