...
def getWordScore(word, n):
    ...
    for w in word:
        if w != '' or (w in SCRABBLE_LETTER_VALUES):
            total += SCRABBLE_LETTER_VALUES[w]
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if isValidWord(userinput, hand, wordList) == False:
                ...
                total += wordscore
                print('"%s" earned %d points. Total: %d points' % (userinput, wordscore, total))
                                # Update the hand
                hand = updateHand(hand, userinput)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    else:
        # This is else condition for 'while' :). Used it property for the 1st time.
        print('Run out of letters. Total score: %d points.' % total)
    
    # Lastly, if the user has explicitely quit, then show them the total score.
    if userinput == '.':
